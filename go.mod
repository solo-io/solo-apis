...
require (
	...
	github.com/solo-io/solo-kit v0.41.1
...
replace (
	...
	// v0.8.0 drops install.istio.io clients; ILM still needs them
	github.com/solo-io/external-apis => github.com/solo-io/external-apis v0.7.0
	...
	// Keep pdata and pprofile on matching collector releases; mismatched versions fail to compile
	// (e.g. pprofile v0.128 expects pdata v1.34 APIs while the module graph may select pdata v1.42+).
	go.opentelemetry.io/collector/pdata => go.opentelemetry.io/collector/pdata v1.42.0
	go.opentelemetry.io/collector/pdata/pprofile => go.opentelemetry.io/collector/pdata/pprofile v0.136.0
	...
	istio.io/istio => github.com/solo-io/istio v1.28.4-solo

	// Gloo Mesh <= 2.12.0 needs Kubernetes 1.34 client libraries due to ILM
	// (transitive deps such as Helm otherwise select k8s.io v0.35.x).
	...
	sigs.k8s.io/kind => sigs.k8s.io/kind v0.30.0
)
