...
func (m *ApiProductSpec) Clone() proto.Message {
	...
	if h, ok := interface{}(m.GetTargetRef()).(clone.Cloner); ok {
		target.TargetRef = h.Clone().(*github_com_solo_io_gloo_mesh_solo_apis_client_go_common_gloo_solo_io_v2.TargetRef)
	} else {
		target.TargetRef = proto.Clone(m.GetTargetRef()).(*github_com_solo_io_gloo_mesh_solo_apis_client_go_common_gloo_solo_io_v2.TargetRef)
	}
