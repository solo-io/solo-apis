...
func (m *WasmDeploymentPolicyStatus) Clone() proto.Message {
	...
	if m.GetSelectedWorkloadRefs() != nil {
		target.SelectedWorkloadRefs = make([]*github_com_solo_io_gloo_mesh_solo_apis_client_go_common_gloo_solo_io_v2.WorkloadReference, len(m.GetSelectedWorkloadRefs()))
		for idx, v := range m.GetSelectedWorkloadRefs() {

			if h, ok := interface{}(v).(clone.Cloner); ok {
				target.SelectedWorkloadRefs[idx] = h.Clone().(*github_com_solo_io_gloo_mesh_solo_apis_client_go_common_gloo_solo_io_v2.WorkloadReference)
			} else {
				target.SelectedWorkloadRefs[idx] = proto.Clone(v).(*github_com_solo_io_gloo_mesh_solo_apis_client_go_common_gloo_solo_io_v2.WorkloadReference)
			}
