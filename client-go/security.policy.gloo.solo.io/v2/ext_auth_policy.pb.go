...
type ExtAuthPolicySpec struct {
	...
	// <li>Only one external auth policy can apply to a route.
	// Subsequent policies (sorted by creation time) are ignored and put into a FAILED state.</li>
	// <li>External auth policies are only applied to routes on a VirtualGateway.
	// East-west routes that are configured with the `workloadSelectors` field in the RouteTable cannot be selected.</li></ul>
	...
	// If an external auth policy is applied to a VirtualDestination and another is applied to a Kubernetes Service backing the VD,
