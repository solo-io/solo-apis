...
func (m *ProxyProtocolPolicySpec_ListenerSelector) Clone() proto.Message {
	...
	if h, ok := interface{}(m.GetVirtualGateway()).(clone.Cloner); ok {
		target.VirtualGateway = h.Clone().(*github_com_solo_io_gloo_mesh_solo_apis_client_go_common_gloo_solo_io_v2.ObjectSelector)
	} else {
		target.VirtualGateway = proto.Clone(m.GetVirtualGateway()).(*github_com_solo_io_gloo_mesh_solo_apis_client_go_common_gloo_solo_io_v2.ObjectSelector)
	}
