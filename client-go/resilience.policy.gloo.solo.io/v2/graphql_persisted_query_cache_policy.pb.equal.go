...
// Equal function
func (m *GraphQLPersistedQueryCachePolicyNewStatus) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*GraphQLPersistedQueryCachePolicyNewStatus)
	if !ok {
		that2, ok := that.(GraphQLPersistedQueryCachePolicyNewStatus)
		if ok {
			target = &that2
		} else {
			return false
