...
//
// +kubebuilder:validation:XValidation:rule="has(self.mode) && self.mode == 'SIMPLE' ? has(self.secretName) || (has(self.files) && has(self.files.serverCert)) : true",message="if tls mode is SIMPLE, either secretName or both files.serverCert and files.privateKey must be set"
// +kubebuilder:validation:XValidation:rule="has(self.mode) && self.mode == 'SIMPLE' ? has(self.secretName) || (has(self.files) && has(self.files.privateKey)) : true",message="if tls mode is SIMPLE, either secretName or both files.serverCert and files.privateKey must be set"
// +kubebuilder:validation:XValidation:rule="has(self.mode) && self.mode == 'MUTUAL' ? has(self.secretName) || (has(self.files) && has(self.files.serverCert)) : true",message="if tls mode is MUTUAL, secretName must be set or files and all its subfields must be set"
// +kubebuilder:validation:XValidation:rule="has(self.mode) && self.mode == 'MUTUAL' ? has(self.secretName) || (has(self.files) && has(self.files.privateKey)) : true",message="if tls mode is MUTUAL, secretName must be set or files and all its subfields must be set"
// +kubebuilder:validation:XValidation:rule="has(self.mode) && self.mode == 'MUTUAL' ? has(self.secretName) || (has(self.files) && has(self.files.caCerts)) : true",message="if tls mode is MUTUAL, secretName must be set or files and all its subfields must be set"
// +kubebuilder:validation:XValidation:rule="has(self.mode) && self.mode == 'ISTIO_MUTUAL' ? !has(self.secretName) : true",message="if tls mode is ISTIO_MUTUAL, secretName must not be set"
// +kubebuilder:validation:XValidation:rule="has(self.mode) && self.mode == 'ISTIO_MUTUAL' ? !has(self.files) : true",message="if tls mode is ISTIO_MUTUAL, files must not be set"
...
//
// +kubebuilder:validation:XValidation:rule="has(self.http) && has(self.tls) ? !has(self.appProtocol) || self.appProtocol.upperAscii() == 'HTTPS' : true",message="if listener type is http and tls field is set, appProtocol must be empty or set to HTTPS"
// +kubebuilder:validation:XValidation:rule="has(self.http) && !has(self.tls) ? !has(self.appProtocol) || self.appProtocol.upperAscii() in ['GRPC','GRPC-WEB','HTTP','HTTP_PROXY','HTTP2'] : true",message="if listener type is http and tls field is unset, appProtocol must be empty or set to GRPC, GRPC-WEB, HTTP, HTTP_PROXY, or HTTP2"
//
...
// +kubebuilder:validation:XValidation:rule="!has(self.http) && has(self.tls) ? !has(self.appProtocol) || self.appProtocol.upperAscii() == 'TLS' : true",message="if listener type is tcp or unspecified and tls field is set, appProtocol must be empty or set to TLS"
// +kubebuilder:validation:XValidation:rule="!has(self.http) && !has(self.tls) ? !has(self.appProtocol) || self.appProtocol.upperAscii() in ['TCP','MONGO','REDIS','MYSQL'] : true",message="if listener type is tcp or unspecified and tls field is unset, appProtocol must be empty or set to TCP, MONGO, REDIS, or MYSQL"
