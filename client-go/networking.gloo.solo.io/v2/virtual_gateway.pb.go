...
const (
	...
	// Note that a client certificate is explicitly requested during the TLS handshake. However, the client is not required to send a certificate.
	// If a client certificate is presented, the certificate is validated. To validate client certificates, you must either specify a CA certificate in the `ca_certs` file or add it to your TLS secret as `ca.crt`.
	//
	// </br>**Configuration constraints**: If `mode` is set to this value, you must either set `tlsConfig.secretName` or set `tlsConfig.files` and all its subfields.
	TLSConfig_OPTIONAL_MUTUAL TLSConfig_TLSMode = 5
)
...
// +kubebuilder:validation:XValidation:rule="has(self.mode) && (self.mode == 'MUTUAL' || self.mode == 'OPTIONAL_MUTUAL') ? has(self.secretName) || (has(self.files) && has(self.files.serverCert)) : true",message="If mode is MUTUAL or OPTIONAL_MUTUAL, you must either set secretName or set files and all its subfields."
// +kubebuilder:validation:XValidation:rule="has(self.mode) && (self.mode == 'MUTUAL' || self.mode == 'OPTIONAL_MUTUAL') ? has(self.secretName) || (has(self.files) && has(self.files.privateKey)) : true",message="If mode is MUTUAL or OPTIONAL_MUTUAL, you must either set secretName or set files and all its subfields."
// +kubebuilder:validation:XValidation:rule="has(self.mode) && (self.mode == 'MUTUAL' || self.mode == 'OPTIONAL_MUTUAL') ? has(self.secretName) || (has(self.files) && has(self.files.caCerts)) : true",message="If mode is MUTUAL or OPTIONAL_MUTUAL, you must either set secretName or set files and all its subfields."
...
type TLSConfig struct {
	...
	// <li>If `mode` is `MUTUAL` or `OPTIONAL_MUTUAL`, you must either set `secretName` or set `files` and all its subfields.</li>
...
type TLSConfig_SecretName struct {
	...
	// For mutual TLS or optional mutual TLS, you can provide `cacert: <CACertificate>` and `crl: <CertificateRevocationList>`
...
// which might require upgrading your Istio Helm installation.
...
type TLSConfig_Files struct {
	...
	// <li>This field is required if `tlsMode` is `SIMPLE`, `MUTUAL`, or `OPTIONAL_MUTUAL`, and `secretName` is not set.</li>
	...
	// </br>**Configuration constraints**:<ul>
	// <li>This field must be empty if `tlsMode` is `ISTIO_MUTUAL`.</li>
	// <li>This field is required if `tlsMode` is `MUTUAL` or `OPTIONAL_MUTUAL`, and `secretName` is not set.</li>
	// <li>This field is ignored if `tlsMode` is `PASSTHROUGH` or `AUTO_PASSTHROUGH`.</li></ul>
...
var file_github_com_solo_io_gloo_mesh_solo_apis_api_gloo_solo_io_networking_v2_virtual_gateway_proto_rawDesc = []byte{
	...
	0x5f, 0x74, 0x79, 0x70, 0x65, 0x22, 0x98, 0x07, 0x0a, 0x09, 0x54, 0x4c, 0x53, 0x43, 0x6f, 0x6e,
	...
