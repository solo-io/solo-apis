...
func (m *VirtualGatewayStatus) Clone() proto.Message {
	...
	if m.GetWorkloads() != nil {
		target.Workloads = make([]*github_com_solo_io_gloo_mesh_solo_apis_client_go_common_gloo_solo_io_v2.ObjectReference, len(m.GetWorkloads()))
		for idx, v := range m.GetWorkloads() {

			if h, ok := interface{}(v).(clone.Cloner); ok {
				target.Workloads[idx] = h.Clone().(*github_com_solo_io_gloo_mesh_solo_apis_client_go_common_gloo_solo_io_v2.ObjectReference)
			} else {
				target.Workloads[idx] = proto.Clone(v).(*github_com_solo_io_gloo_mesh_solo_apis_client_go_common_gloo_solo_io_v2.ObjectReference)
			}
