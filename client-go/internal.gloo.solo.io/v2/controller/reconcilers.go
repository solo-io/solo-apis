...
type SpireRegistrationEntryReconciler interface {
	ReconcileSpireRegistrationEntry(obj *internal_gloo_solo_io_v2.SpireRegistrationEntry) (reconcile.Result, error)
}
...
type SpireRegistrationEntryReconcilerFuncs struct {
	OnReconcileSpireRegistrationEntry         func(obj *internal_gloo_solo_io_v2.SpireRegistrationEntry) (reconcile.Result, error)
	OnReconcileSpireRegistrationEntryDeletion func(req reconcile.Request) error
}

func (f *SpireRegistrationEntryReconcilerFuncs) ReconcileSpireRegistrationEntry(obj *internal_gloo_solo_io_v2.SpireRegistrationEntry) (reconcile.Result, error) {
...
type SpireRegistrationEntryFinalizer interface {
	...
	// finalize the object before it is deleted.
	// Watchers created with a finalizing handler will a
	FinalizeSpireRegistrationEntry(obj *internal_gloo_solo_io_v2.SpireRegistrationEntry) error
}
...
func NewSpireRegistrationEntryReconcileLoop(name string, mgr manager.Manager, options reconcile.Options) SpireRegistrationEntryReconcileLoop {
	return &spireRegistrationEntryReconcileLoop{
		// empty cluster indicates this reconciler is built for the local cluster
		loop: reconcile.NewLoop(name, "", mgr, &internal_gloo_solo_io_v2.SpireRegistrationEntry{}, options),
	}
}
