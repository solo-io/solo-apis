...
func (m *MeshSpec) HashUnique(hasher hash.Hash64) (uint64, error) {
	...
	if h, ok := interface{}(m.GetSpire()).(safe_hasher.SafeHasher); ok {
		if _, err = hasher.Write([]byte("Spire")); err != nil {
			return 0, err
		}
		if _, err = h.Hash(hasher); err != nil {
			return 0, err
		...
		if fieldValue, err := hashstructure.Hash(m.GetSpire(), nil); err != nil {
			return 0, err
		} else {
			if _, err = hasher.Write([]byte("Spire")); err != nil {
				return 0, err
