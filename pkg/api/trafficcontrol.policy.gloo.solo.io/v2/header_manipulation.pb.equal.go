...
// Equal function
func (m *HeaderManipulationPolicyNewStatus) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*HeaderManipulationPolicyNewStatus)
	if !ok {
		that2, ok := that.(HeaderManipulationPolicyNewStatus)
		if ok {
			target = &that2
		} else {
			return false
