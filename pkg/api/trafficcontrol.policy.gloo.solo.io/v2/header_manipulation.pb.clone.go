...
func (m *HeaderManipulationPolicySpec) Clone() proto.Message {
	...
	if m.GetApplyToRouteDestinations() != nil {
		target.ApplyToRouteDestinations = make([]*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.RouteDestinationSelector, len(m.GetApplyToRouteDestinations()))
		for idx, v := range m.GetApplyToRouteDestinations() {

			if h, ok := interface{}(v).(clone.Cloner); ok {
				target.ApplyToRouteDestinations[idx] = h.Clone().(*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.RouteDestinationSelector)
			} else {
				target.ApplyToRouteDestinations[idx] = proto.Clone(v).(*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.RouteDestinationSelector)
			}
