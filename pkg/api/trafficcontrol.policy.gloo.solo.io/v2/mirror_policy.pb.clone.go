...
// Clone function
func (m *MirrorPolicyNewStatus) Clone() proto.Message {
	var target *MirrorPolicyNewStatus
	if m == nil {
		return target
	}
	target = &MirrorPolicyNewStatus{}
...
func (m *MirrorPolicySpec_Config) Clone() proto.Message {
	...
	if h, ok := interface{}(m.GetDestination()).(clone.Cloner); ok {
		target.Destination = h.Clone().(*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.DestinationReference)
	} else {
		target.Destination = proto.Clone(m.GetDestination()).(*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.DestinationReference)
	}
