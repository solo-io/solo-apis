...
func (m *ExternalEndpointSpec) Clone() proto.Message {
	...
	if h, ok := interface{}(m.GetLocality()).(clone.Cloner); ok {
		target.Locality = h.Clone().(*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.Locality)
	} else {
		target.Locality = proto.Clone(m.GetLocality()).(*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.Locality)
	}
...
// Clone function
func (m *ExternalEndpointNewStatus) Clone() proto.Message {
	var target *ExternalEndpointNewStatus
	if m == nil {
		return target
	}
	target = &ExternalEndpointNewStatus{}
