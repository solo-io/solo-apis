...
// Equal function
func (m *RouteTableNewStatus) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*RouteTableNewStatus)
	if !ok {
		that2, ok := that.(RouteTableNewStatus)
		if ok {
			target = &that2
		} else {
			return false
	...
	for k, v := range m.GetAppliedRoutePolicies() {
		...
		if v != target.GetAppliedRoutePolicies()[k] {
			return false
		}

	}

	if m.GetParentRouteTables() != target.GetParentRouteTables() {
		return false
	}
	...
	if m.GetAllowedVirtualGateways() != target.GetAllowedVirtualGateways() {
		return false
	}
