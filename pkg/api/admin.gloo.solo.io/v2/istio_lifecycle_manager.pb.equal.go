...
func (m *IstioLifecycleManagerSpec) Equal(that interface{}) bool {
	...
	if h, ok := interface{}(m.GetUpgradeStrategy()).(equality.Equalizer); ok {
		if !h.Equal(target.GetUpgradeStrategy()) {
			return false
		}
	} else {
		if !proto.Equal(m.GetUpgradeStrategy(), target.GetUpgradeStrategy()) {
			return false
		}
	}
...
// Equal function
func (m *IstioLifecycleManagerSpec_UpgradeStrategy) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*IstioLifecycleManagerSpec_UpgradeStrategy)
	if !ok {
		that2, ok := that.(IstioLifecycleManagerSpec_UpgradeStrategy)
		if ok {
			target = &that2
		} else {
			return false
	...
	if m.GetAlwaysUseCanaryUpgrade() != target.GetAlwaysUseCanaryUpgrade() {
		return false
	}
...
func (m *IstioLifecycleManagerStatus_ClusterStatuses_InstallationStatus) Equal(that interface{}) bool {
	...
	if strings.Compare(m.GetGeneratedRevision(), target.GetGeneratedRevision()) != 0 {
		return false
	}
