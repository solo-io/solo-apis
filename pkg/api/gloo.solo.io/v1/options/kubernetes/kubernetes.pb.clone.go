...
func (m *UpstreamSpec) Clone() proto.Message {
	...
	if h, ok := interface{}(m.GetSubsetSpec()).(clone.Cloner); ok {
		target.SubsetSpec = h.Clone().(*github_com_solo_io_solo_apis_pkg_api_gloo_solo_io_v1_options.SubsetSpec)
	} else {
		target.SubsetSpec = proto.Clone(m.GetSubsetSpec()).(*github_com_solo_io_solo_apis_pkg_api_gloo_solo_io_v1_options.SubsetSpec)
	}
