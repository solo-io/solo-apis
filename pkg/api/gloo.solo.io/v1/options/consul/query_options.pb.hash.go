...
func (m *QueryOptions) Hash(hasher hash.Hash64) (uint64, error) {
	...
	var err error
	if _, err = hasher.Write([]byte("consul.options.gloo.solo.io.github.com/solo-io/solo-apis/pkg/api/gloo.solo.io/v1/options/consul.QueryOptions")); err != nil {
		return 0, err
	}
	...
	if h, ok := interface{}(m.GetUseCache()).(safe_hasher.SafeHasher); ok {
		...
		if val, err := hashstructure.Hash(m.GetUseCache(), nil); err != nil {
			return 0, err
		} else {
