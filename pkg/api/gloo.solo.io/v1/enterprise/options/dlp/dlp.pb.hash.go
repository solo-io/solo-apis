...
func (m *Action) Hash(hasher hash.Hash64) (uint64, error) {
	...
	if h, ok := interface{}(m.GetKeyValueAction()).(safe_hasher.SafeHasher); ok {
		...
		if val, err := hashstructure.Hash(m.GetKeyValueAction(), nil); err != nil {
			return 0, err
		} else {
			if err := binary.Write(hasher, binary.LittleEndian, val); err != nil {
				return 0, err
...
func (m *KeyValueAction) Hash(hasher hash.Hash64) (uint64, error) {
	...
	var err error
	if _, err = hasher.Write([]byte("dlp.options.gloo.solo.io.github.com/solo-io/solo-apis/pkg/api/gloo.solo.io/v1/enterprise/options/dlp.KeyValueAction")); err != nil {
		return 0, err
	}
