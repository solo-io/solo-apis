...
func (m *Settings) Hash(hasher hash.Hash64) (uint64, error) {
	...
	var err error
	if _, err = hasher.Write([]byte("caching.options.gloo.solo.io.github.com/solo-io/solo-apis/pkg/api/gloo.solo.io/v1/enterprise/options/caching.Settings")); err != nil {
		return 0, err
	}
	...
	if h, ok := interface{}(m.GetCachingServiceRef()).(safe_hasher.SafeHasher); ok {
		...
		if val, err := hashstructure.Hash(m.GetCachingServiceRef(), nil); err != nil {
			return 0, err
		} else {
	...
	if h, ok := interface{}(m.GetMaxPayloadSize()).(safe_hasher.SafeHasher); ok {
		...
		if val, err := hashstructure.Hash(m.GetMaxPayloadSize(), nil); err != nil {
			return 0, err
		} else {
			if err := binary.Write(hasher, binary.LittleEndian, val); err != nil {
				return 0, err
