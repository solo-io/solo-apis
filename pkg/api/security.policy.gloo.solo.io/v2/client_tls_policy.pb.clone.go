...
// Clone function
func (m *ClientTLSPolicyStatusNew) Clone() proto.Message {
	var target *ClientTLSPolicyStatusNew
	if m == nil {
		return target
	}
	target = &ClientTLSPolicyStatusNew{}
...
func (m *ClientTLSPolicyReport) Clone() proto.Message {
	...
	if h, ok := interface{}(m.GetWorkspaces()).(clone.Cloner); ok {
		target.Workspaces = h.Clone().(*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.Report)
	} else {
		target.Workspaces = proto.Clone(m.GetWorkspaces()).(*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.Report)
	}
