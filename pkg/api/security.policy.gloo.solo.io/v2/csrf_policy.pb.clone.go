...
// Clone function
func (m *CSRFPolicyNewStatus) Clone() proto.Message {
	var target *CSRFPolicyNewStatus
	if m == nil {
		return target
	}
	target = &CSRFPolicyNewStatus{}
...
func (m *CSRFPolicySpec_Config) Clone() proto.Message {
	...
	if m.GetAdditionalOrigins() != nil {
		target.AdditionalOrigins = make([]*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.StringMatch, len(m.GetAdditionalOrigins()))
		for idx, v := range m.GetAdditionalOrigins() {

			if h, ok := interface{}(v).(clone.Cloner); ok {
				target.AdditionalOrigins[idx] = h.Clone().(*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.StringMatch)
			} else {
				target.AdditionalOrigins[idx] = proto.Clone(v).(*github_com_solo_io_solo_apis_pkg_api_common_gloo_solo_io_v2.StringMatch)
			}
