...
// Equal function
func (m *ClientTLSPolicyStatusNew) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*ClientTLSPolicyStatusNew)
	if !ok {
		that2, ok := that.(ClientTLSPolicyStatusNew)
		if ok {
			target = &that2
		} else {
			return false
...
func (m *ClientTLSPolicyReport) Equal(that interface{}) bool {
	...
	if h, ok := interface{}(m.GetWorkspaces()).(equality.Equalizer); ok {
		if !h.Equal(target.GetWorkspaces()) {
			return false
		}
	} else {
		if !proto.Equal(m.GetWorkspaces(), target.GetWorkspaces()) {
			return false
		}
	}
